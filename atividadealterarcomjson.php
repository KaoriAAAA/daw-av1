...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'salvar_pergunta_alterada') {
        requireAdmin();
        $codigo_original = $_POST['codigo_original'];
        $novo_enunciado = trim($_POST['novo_enunciado']);
        $tipo = $_POST['tipo'];
        ...
        foreach ($linhas as $i => $linha) {
            ...
            if (trim($col[0]) === $codigo_original) {
                if ($tipo === 'Multipla Escolha') {
                    ...
                    $op4 = trim($_POST['opcao4']);
                    $opcaocerta = $_POST['opcaocerta'];
                    $novaLista[] = "{$novo_enunciado};Multipla Escolha;{$op1};{$op2};{$op3};{$op4};{$opcaocerta}";
                } else {
                    $novaLista[] = "{$novo_enunciado};Discursiva;;;;;;";
                }
            } else {
                $novaLista[] = $linha;
            }
        ...
        header('Content-Type: application/json');
        echo json_encode(['success' => true, 'message' => 'Pergunta alterada com sucesso']);
        exit;
    }
...
<ul>
    ...
    <li><a href="?page=alterar_pergunta_ajax">Alterar Pergunta (AJAX)</a></li>
...
<?php if ($page === 'alterar_pergunta_ajax' && isset($_SESSION['username'])): requireAdmin(); ?>
<h3>Alterar Pergunta (AJAX)</h3>

<div id="buscar-pergunta">
    <h4>Buscar Pergunta</h4>
    <form id="form-buscar">
        <label>Código da Pergunta (Enunciado):</label><br>
        <input type="text" id="codigo-pergunta" name="codigo" required style="width: 400px;">
        <button type="submit">Buscar Pergunta</button>
    </form>
</div>

<div id="form-alterar" style="display:none;">
    <h4>Alterar Pergunta</h4>
    <form id="form-salvar">
        <input type="hidden" id="codigo-original" name="codigo_original">
        <input type="hidden" id="tipo-pergunta" name="tipo">
        
        <label>Enunciado:</label><br>
        <input type="text" id="novo-enunciado" name="novo_enunciado" required style="width: 400px;"><br><br>
        
        <div id="opcoes-multipla" style="display:none;">
            <label>Opção A:</label><br>
            <input type="text" id="opcao1" name="opcao1" style="width: 300px;"><br>
            
            <label>Opção B:</label><br>
            <input type="text" id="opcao2" name="opcao2" style="width: 300px;"><br>
            
            <label>Opção C:</label><br>
            <input type="text" id="opcao3" name="opcao3" style="width: 300px;"><br>
            
            <label>Opção D:</label><br>
            <input type="text" id="opcao4" name="opcao4" style="width: 300px;"><br><br>
            
            <label>Opção Correta:</label><br>
            <select id="opcao-certa" name="opcaocerta">
                <option value="a)">A</option>
                <option value="b)">B</option>
                <option value="c)">C</option>
                <option value="d)">D</option>
...
function buscarPergunta() {
    const codigo = document.getElementById('codigo-pergunta').value;
    ...
    fetch('', {
        ...
        body: 'acao=buscar_pergunta&codigo=' + encodeURIComponent(codigo)
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            carregarFormulario(data.pergunta);
            mensagemDiv.innerHTML = '<p style="color:green;">Pergunta encontrada!</p>';
        } else {
            mensagemDiv.innerHTML = '<p style="color:red;">' + data.message + '</p>';
        }
    })
    .catch(error => {
        mensagemDiv.innerHTML = '<p style="color:red;">Erro ao buscar pergunta</p>';
        console.error('Error:', error);
    });
}

function carregarFormulario(pergunta) {
    document.getElementById('codigo-original').value = pergunta.enunciado;
    document.getElementById('tipo-pergunta').value = pergunta.tipo;
    document.getElementById('novo-enunciado').value = pergunta.enunciado;
    
    if (pergunta.tipo === 'Multipla Escolha') {
        document.getElementById('opcoes-multipla').style.display = 'block';
        document.getElementById('opcao1').value = pergunta.opcao1;
        document.getElementById('opcao2').value = pergunta.opcao2;
        document.getElementById('opcao3').value = pergunta.opcao3;
        document.getElementById('opcao4').value = pergunta.opcao4;
        document.getElementById('opcao-certa').value = pergunta.opcaocerta;
    } else {
        document.getElementById('opcoes-multipla').style.display = 'none';
    }
    
    document.getElementById('buscar-pergunta').style.display = 'none';
    document.getElementById('form-alterar').style.display = 'block';
}

function salvarPergunta() {
    const formData = new FormData(document.getElementById('form-salvar'));
    formData.append('acao', 'salvar_pergunta_alterada');
    ...
    .then(data => {
        if (data.success) {
            ...
            mensagemDiv.innerHTML = '<p style="color:red;">Erro ao salvar pergunta</p>';
        }
    })
    .catch(error => {
        mensagemDiv.innerHTML = '<p style="color:red;">Erro ao salvar pergunta</p>';
...
function cancelarAlteracao() {
    document.getElementById('buscar-pergunta').style.display = 'block';
    document.getElementById('form-alterar').style.display = 'none';
    document.getElementById('mensagem').innerHTML = '';
    document.getElementById('codigo-pergunta').value = '';
