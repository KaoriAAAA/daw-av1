...
if(isset($_POST['acao'])){
    ...
    if($acao==='login'){
        ...
        $user = $_POST['username'] ?? '';
        $pass = $_POST['password'] ?? '';
        $stmt = $conn->prepare("SELECT id, nome, tipo_de_acesso FROM usuarios WHERE nome = ?");
        ...
        if($stmt->num_rows === 1){
            ...
            $_SESSION['nome'] = $nome;
            $_SESSION['role'] = ($tipo == 2) ? 'admin' : 'funcionario';
    ...
    if($acao==='listar_usuarios'){
        requireAdmin();
        $res = $conn->query("SELECT id, nome, cpf, e_mail, telefone, tipo_de_acesso FROM usuarios");
        $usuarios = [];
        while($row = $res->fetch_assoc()){
            $usuarios[] = [
                'username' => $row['nome'],
                'nome' => $row['nome'],
                'role' => ($row['tipo_de_acesso'] == 2) ? 'admin' : 'funcionario'
            ];
        }
    ...
    if($acao==='criar_usuario'){ 
        ...
        $stmt = $conn->prepare("INSERT INTO usuarios (nome, cpf, e_mail, telefone, tipo_de_acesso) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssi", $nome, $cpf, $email, $telefone, $tipo_acesso);
        $ok = $stmt->execute();
        $stmt->close();
        ...
        if($ok){
            ...
            echo json_encode(['sucesso'=>false,'mensagem'=>'Erro ao criar usuário: '.$conn->error]);
        }
        exit;
    }
    ...
    if($acao==='apagar_usuario'){ 
        ...
        $ok = $stmt->execute();
        $stmt->close();
        echo json_encode(['sucesso'=>$ok,'mensagem'=>$ok?'Usuário apagado':'Erro ao apagar usuário']);
        exit;
    }
    ...
    if($acao==='editar_usuario'){
        ...
        $stmt = $conn->prepare("UPDATE usuarios SET nome=?, cpf=?, e_mail=?, telefone=?, tipo_de_acesso=? WHERE nome=?");
        $stmt->bind_param("ssssss", $nome, $cpf, $email, $telefone, $tipo_acesso, $original);
        $ok = $stmt->execute();
        $stmt->close();
        echo json_encode(['sucesso'=>$ok,'mensagem'=>$ok?'Usuário alterado':'Erro ao alterar usuário']);
        exit;
    }
...
async function loadUsuarios(){
    ...
    if(res.sucesso){
        ...
        html+="<input id='novoCPF' placeholder='CPF'> <input id='novoEmail' placeholder='E-mail'> <input id='novoTelefone' placeholder='Telefone'>";
...
async function criarUsuario(){
    ...
    const res=await ajaxPost({acao:'criar_usuario',username:user,senha:pass,nome:nome,cpf:cpf,email:email,telefone:telefone,role:role});
    showMsg(res.mensagem,res.sucesso);
...
async function criarPergunta(){
    const enun=document.getElementById('pergEnunciado').value;
    const tipo=document.getElementById('pergTipo').value;
    let data={acao:'criarpergunta',enunciado:enun,tipo:tipo};
    if(tipo==='Multipla Escolha'){
...
async function responderPerguntas(){
    const res=await ajaxPost({acao:'listar_perguntas'});
    if(res.sucesso){
        let html="<h3>Responder Perguntas</h3>";
        res.perguntas.forEach(p=>{
            html+="<div><b>"+p.enunciado+"</b><br>";
            if(p.tipo==='Discursiva'){
                html+="<textarea id='resp_"+p.enunciado+"'></textarea><br>";
            }else{
                html+="<input type='radio' name='r_"+p.enunciado+"' value='a)'>"+p.opcoes.split(' ')[0]+"<br>";
                html+="<input type='radio' name='r_"+p.enunciado+"' value='b)'>"+p.opcoes.split(' ')[1]+"<br>";
                html+="<input type='radio' name='r_"+p.enunciado+"' value='c)'>"+p.opcoes.split(' ')[2]+"<br>";
                html+="<input type='radio' name='r_"+p.enunciado+"' value='d)'>"+p.opcoes.split(' ')[3]+"<br>";
            }
            html+="<button onclick='enviarResposta(\""+p.enunciado+"\",\""+p.tipo+"\")'>Enviar</button></div><hr>";
        });
        document.getElementById('conteudo').innerHTML=html;
    }
}
async function enviarResposta(enunciado,tipo){
    let resp="";
    if(tipo==='Discursiva') resp=document.getElementById('resp_'+enunciado).value;
    else{
        const radios=document.getElementsByName('r_'+enunciado);
        for(let r of radios){ if(r.checked) resp=r.value; }
    }
    const res=await ajaxPost({acao:'responder',enunciado:enunciado,resposta:resp});
    showMsg(res.mensagem,res.sucesso);
}
async function verMinhasRespostas(){
    const res=await ajaxPost({acao:'ver_respostas'});
    if(res.sucesso){
        let html="<h3>Minhas Respostas</h3><table><tr><th>Pergunta</th><th>Resposta</th><th>Data</th></tr>";
